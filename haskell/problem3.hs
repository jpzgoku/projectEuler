...
prime_factor x 
    ...
    where only_primes (x:xs)
              | (x:xs) == divide_non_primes (x:xs) = (x:xs) 
              | otherwise = only_primes (divide_non_primes (x:xs))
              where divide_non_primes (x:xs) = conc ([pair_multiples x | x <- (x:xs)])
